...
이는 ViT의 입력 크기가 Mnist 이미지 데이터와 맞지 않기 때문인 것으로 보인다. Vit는 16 픽셀을 1 패치로 사용하고 총 256x256 크기의 이미지를 입력으로 받는다. 하지만 32 픽셀인 MNist를 학습 할 떄 입력 차원을 줄이면 패치가 제 기능을 하기 어렵고, 그대로 두면 빈공간이 많아 진다. 
